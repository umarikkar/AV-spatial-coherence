...
def main():
    ...
    for seq in seq_list:
        ...
        loader = DataLoader(data_all, batch_size=bs, shuffle=False, num_workers=0, pin_memory=True)

        fol_name = conf.filenames['net_folder_path']

        # epoch_settings = np.linspace(4,16,4).astype('int')

        epoch_settings = np.linspace(4, 48, 12).astype('int')

        epoch_settings = [20]

        tolerance_list = [10,20,30,40,50,60]
